...
;(async () => {
  ...
  const makeHeader = (index, title, position) => {
    ...
    const insertposition =
      position +
      (position == "after" ? "end" : position == "before" ? "begin" : "")
  ...
  await new Promise((resolve) => {
    ...
    const tempInterval = setInterval(() => {
      ...
      const updateSubmitBtn = () => {
        ...
        const countEnabled = switchStateLookup.filter(
          (state) => state === true
        ).length
      ...
      rows.forEach((row, index) => {
        ...
        toggleInput.addEventListener("change", (event) => {
          ...
          const targetGenderOptions = document.querySelectorAll(
            `input[id^="gender"][rowindex="${thisIndex}"]`
          )
