...
const insertMaster = (master, isMarried) => {
    ...
    masterFields.birthdate.month.value = Number(master.birthdate.month).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })
    masterFields.birthdate.day.value = Number(master.birthdate.day).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })
    // gender (default to male)
    masterFields.gender.querySelector(`input[value=${master.gender}]`).click()
    // is married?
    masterFields.isMarried.querySelector(`input[value="${isMarried ? 1 : 0}"]`).click()
}
...
const insertSpouse = (spouse) => {
    ...
    const tempInterval = setInterval(() => {
        ...
        spouseFields.birthdate.month.value = Number(spouse.birthdate.month).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })
        spouseFields.birthdate.day.value = Number(spouse.birthdate.day).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })
